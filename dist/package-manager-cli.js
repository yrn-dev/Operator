...
import { getLatestOperatorRelease, isNewerPackageVersion } from "./utils/version-check.js";
...
function getPackageCommandUsage(command) {
    switch (command) {
        ...
        case "update":
            return `${APP_NAME} update [source|self|opr] [--self|--extensions|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
        case "list":
...
Update opr and installed packages.

Options:
  --self                  Update opr only (default when no target is given)
  --extensions            Update installed packages only
  --all                   Update opr and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall opr even if the current version is latest

Short forms:
  ${APP_NAME} update                Update opr only
  ${APP_NAME} update --all          Update opr and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update opr            Update opr only (self works as alias to opr)
`);
            return;
        case "list":
            console.log(`${chalk.bold("Usage:")}
...
function parsePackageCommand(args) {
    ...
    if (command === "update") {
        ...
        if (extensionFlagSource) {
            ...
            const sourceIsSelf = source === "self" || source === "opr";
            if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(npmCommand, updatePackageTarget = PACKAGE_NAME) {
    ...
    if (entrypoint) {
        console.error("");
        console.error(`Location of opr executable: ${entrypoint}`);
    }
}
...
async function getSelfUpdatePlan(force) {
    let latestRelease;
    try {
        latestRelease = await getLatestOperatorRelease(VERSION);
    }
    catch (error) {
