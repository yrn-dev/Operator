...
import { getOperatorUserAgent } from "../../utils/operator-user-agent.js";
import { killTrackedDetachedChildren } from "../../utils/shell.js";
import { ensureTool } from "../../utils/tools-manager.js";
import { checkForNewOperatorVersion } from "../../utils/version-check.js";
...
export class InteractiveMode {
    ...
    workingMessageIndex = 0;
    workingMessageTimer = undefined;
    defaultWorkingMessage = "Cooking...";
    ...
    async run() {
        ...
        checkForNewOperatorVersion(this.version).then((newRelease) => {
            if (newRelease) {
    ...
    async checkTmuxKeyboardSetup() {
        ...
        if (extendedKeysFormat === "xterm") {
            return "tmux extended-keys-format is xterm. OPR works best with csi-u. Add `set -g extended-keys-format csi-u` to ~/.tmux.conf and restart tmux.";
        }
        return undefined;
    }
    /**
    ...
    getWorkingLoaderMessage() {
        return this.workingMessage ?? this.workingMessages[this.workingMessageIndex];
    }
    createWorkingLoader() {
        ...
        // Циклически меняем сообщения каждые 3 секунды
        this.workingMessageIndex = 0;
        this.workingMessageTimer = setInterval(() => {
            this.workingMessageIndex = (this.workingMessageIndex + 1) % this.workingMessages.length;
            if (this.loadingAnimation) {
                this.loadingAnimation.setMessage(this.workingMessages[this.workingMessageIndex]);
            }
        }, 3000);
        return loader;
    }
    stopWorkingLoader() {
        if (this.workingMessageTimer) {
            clearInterval(this.workingMessageTimer);
            this.workingMessageTimer = undefined;
        }
        if (this.loadingAnimation) {
            this.loadingAnimation.stop();
            this.loadingAnimation = undefined;
    ...
    resetExtensionUI() {
        ...
        if (this.loadingAnimation) {
            this.loadingAnimation.setMessage(`${this.workingMessages[this.workingMessageIndex]} (${keyText("app.interrupt")} to interrupt)`);
        }
    ...
    createExtensionUIContext() {
        return {
            ...
            setWorkingMessage: (message) => {
                ...
                if (this.loadingAnimation) {
                    this.loadingAnimation.setMessage(message ?? this.workingMessages[this.workingMessageIndex]);
                }
            },
    ...
    setupKeyHandlers() {
        ...
        this.defaultEditor.onAction("app.reasoning.cycle", () => this.cycleThinkingLevel());
        ...
        this.defaultEditor.onAction("app.reasoning.toggle", () => this.toggleThinkingBlockVisibility());
    ...
    async handleClipboardImagePaste() {
        try {
            ...
            const fileName = `opr-clipboard-${crypto.randomUUID()}.${ext}`;
    ...
    renderProjectTrustWarningIfNeeded() {
        ...
        this.chatContainer.addChild(new Text(theme.fg("warning", `This project is not trusted. Project ${CONFIG_DIR_NAME} resources and packages are ignored. Use /trust to save a trust decision, then restart opr.`), 1, 0));
    }
    async getUserInput() {
    ...
    uncaughtCrash(error) {
        ...
        console.error("opr exiting due to uncaughtException:");
        console.error(error);
        process.exit(1);
    }
    /**
    ...
    registerSignalHandlers() {
        ...
        // in operator) leaves the terminal in raw mode with no cursor.
    ...
    cycleThinkingLevel() {
        ...
        if (newLevel === undefined) {
            this.showStatus("Current model does not support reasoning");
        }
        else {
            this.footer.invalidate();
            this.updateEditorBorderColor();
    ...
    async cycleModel(direction) {
        try {
            ...
            if (result === undefined) {
                ...
                this.footer.invalidate();
                this.updateEditorBorderColor();
                const thinkingStr = result.model.reasoning && result.thinkingLevel !== "off" ? ` (reasoning: ${result.thinkingLevel})` : "";
    ...
    async openExternalEditor() {
        ...
        const tmpFile = path.join(os.tmpdir(), `opr-editor-${Date.now()}.md`);
        try {
            ...
            process.stdout.write(`Launching external editor: ${editorCmd}\nOPR will resume when the editor exits.\n`);
    ...
    handleStatusCommand() {
        ...
        if (!model) {
            this.showWarning("Модель не выбрана.");
            return;
        }
        ...
        const usedTokens = ctx?.tokens ?? null;
        ...
        let percentLabel;
        let percentColor;
        if (percent === null || usedTokens === null || contextWindow <= 0) {
            bar = theme.fg("dim", "░".repeat(barWidth));
            percentLabel = "неизвестно (после сжатия контекста)";
            percentColor = "dim";
        }
        else {
            const filled = Math.max(0, Math.min(barWidth, Math.round((percent / 100) * barWidth)));
            percentColor = percent > 90 ? "error" : percent > 70 ? "warning" : "success";
            bar = theme.fg(percentColor, "█".repeat(filled)) + theme.fg("dim", "░".repeat(barWidth - filled));
            percentLabel = `${percent.toFixed(1)}%`;
        }
        const usingSubscription = this.session.modelRegistry.isUsingOAuth(model);
        const costLine = stats.cost > 0 || usingSubscription
            ? `$${stats.cost.toFixed(4)}${usingSubscription ? " (подписка)" : ""}`
            : "$0";
        const thinkingLine = model.reasoning
            ? ` · рассуждение: ${this.session.state.thinkingLevel || "off"}`
            : "";
        const lines = [
            theme.fg("accent", "● Статус сессии"),
            "",
            `${theme.fg("dim", "Модель:      ")}${model.name || model.id}${thinkingLine}`,
            `${theme.fg("dim", "Провайдер:   ")}${providerName}`,
            `${theme.fg("dim", "Контекст:    ")}${bar}  ${theme.fg(percentColor, percentLabel)}`,
            `${theme.fg("dim", "Использовано:")} ${usedTokens !== null
                ? `${fmt(usedTokens)} / ${fmt(contextWindow)} токенов`
                : `окно ${fmt(contextWindow)} токенов`}`,
            "",
            `${theme.fg("dim", "За сессию:")} вход ${fmt(stats.tokens.input)} · выход ${fmt(stats.tokens.output)} · кэш-чтение ${fmt(stats.tokens.cacheRead)} · кэш-запись ${fmt(stats.tokens.cacheWrite)}`,
            `${theme.fg("dim", "Всего токенов:")} ${fmt(stats.tokens.total)}  ${theme.fg("dim", "· Стоимость:")} ${costLine}`,
            `${theme.fg("dim", "Сообщений:")} ${stats.totalMessages} (пользователь ${stats.userMessages}, ассистент ${stats.assistantMessages}) · вызовов инструментов: ${stats.toolCalls}`,
        ];
        this.chatContainer.addChild(new Spacer(1));
    ...
    showNewVersionNotification(release) {
        ...
        const changelogUrl = "https://opr.local/changelog";
        ...
        this.showSelector((done) => {
            const selector = new TrustSelectorComponent({
                ...
                onSelect: (selection) => {
                    ...
                    this.showStatus(`Saved trust decision: ${selection.trusted ? "trusted" : "untrusted"}. Restart opr for this to take effect.`);
                },
                onCancel: () => {
                    done();
                    this.ui.requestRender();
            ...
            const selector = new ModelSelectorComponent(this.ui, this.session.model, this.settingsManager, this.session.modelRegistry, this.session.scopedModels, async (model) => {
                try {
                    if (!this.session.modelRegistry.hasConfiguredAuth(model)) {
                        ...
                        await this.showApiKeyLoginDialog(model.provider, this.session.modelRegistry.getProviderDisplayName(model.provider), model);
                        return;
                    }
                    await this.session.setModel(model);
                    this.footer.invalidate();
                    ...
                    this.showStatus(`Model: ${displayName}`);
                    void this.maybeWarnAboutAnthropicSubscriptionAuth(model);
                    this.checkDaxnutsEasterEgg(model);
                }
                catch (error) {
    ...
    async showApiKeyLoginDialog(providerId, providerName, targetModel) {
        ...
        try {
            ...
            if (targetModel) {
                ...
                this.footer.invalidate();
                this.updateEditorBorderColor();
                const displayName = targetModel.id;
                this.showStatus(`Model: ${displayName}`);
            }
        }
        catch (error) {
    ...
    async handleExportCommand(text) {
        ...
        try {
            // Default export path
            const filePath = outputPath || `${this.sessionManager.getSessionName() || "session"}.opr.json`;
            const fullPath = filePath.startsWith("/") ? filePath : `${process.cwd()}/${filePath}`;
            // Export session as JSON
            const exportData = this.session.exportToJson(fullPath);
            this.showStatus(`Session exported to: ${exportData}`);
        }
        catch (error) {
    ...
    handleHotkeysCommand() {
        ...
        const cycleThinkingLevel = this.getAppKeyDisplay("app.reasoning.cycle");
        const cycleModelForward = this.getAppKeyDisplay("app.model.cycleForward");
        const selectModel = this.getAppKeyDisplay("app.model.select");
        const expandTools = this.getAppKeyDisplay("app.tools.expand");
        const toggleThinking = this.getAppKeyDisplay("app.reasoning.toggle");
...
| \`${toggleThinking}\` | Toggle reasoning block visibility |
