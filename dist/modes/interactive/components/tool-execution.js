...
export class ToolExecutionComponent extends Container {
    ...
    render(width) {
        ...
        if (this.hasRendererDefinition() && this.getRenderShell() === "self") {
            const contentLines = this.selfRenderContainer.render(Math.max(1, width - 2));
            ...
            if (contentLines.length > 0) {
                lines.push("");
                lines.push(rail(`╭─ TOOL ${this.toolName}`));
                lines.push(...contentLines.map((line) => `${rail("│ ")}${line}`));
                lines.push(rail("╰"));
            }
