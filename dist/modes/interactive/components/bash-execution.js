...
export class BashExecutionComponent extends Container {
    ...
    constructor(command, ui, excludeFromContext = false) {
        ...
        const header = new Text(theme.fg(colorKey, theme.bold("$ ")) + theme.fg("text", command), 0, 0);
    ...
    render(width) {
        ...
        const color = this.status === "error" ? "error" : this.status === "cancelled" ? "warning" : "bashMode";
        const rail = (text) => theme.fg(color, text);
        return [
            "",
            rail("╭─ RUN"),
    ...
    updateDisplay() {
        ...
        // Command header
        const header = new Text(theme.fg("bashMode", theme.bold("$ ")) + theme.fg("text", this.command), 0, 0);
        this.contentContainer.addChild(header);
        // Output
