...
/**
 * Create bash operations using operator's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want operator's
...
export function createBashToolDefinition(cwd, options) {
    ...
    return {
        ...
        async execute(_toolCallId, { command, timeout }, signal, onUpdate, _ctx) {
            ...
            const output = new OutputAccumulator({ tempFilePrefix: "opr-bash" });
