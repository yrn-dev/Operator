...
export class AgentSession {
    ...
    async _getRequiredRequestAuth(model) {
        ...
        return { apiKey: "dummy", headers: result.headers, env: result.env };
    }
    ...
    dispose() {
        ...
        this._extensionRunner.invalidate("This extension context is stale after session replacement or reload. Do not use a captured opr or command context after ctx.newSession(), ctx.fork(), ctx.switchSession(), or ctx.reload(). For newSession, fork, and switchSession, move post-replacement work into withSession and use the context passed to withSession. For reload, do not use the old context after await ctx.reload().");
     ...
     * - Handles extension commands (registered via operator.registerCommand) immediately, even during streaming
    ...
    async prompt(text, options) {
        ...
        try {
            ...
            // Extension commands manage their own LLM interaction via operator.sendMessage()
    ...
    /**
     * Export the current session as a JSON file (shareable with another user).
     * @param outputPath Target file path.
     * @returns The resolved output file path.
     */
    exportToJson(outputPath) {
        const filePath = resolvePath(outputPath, process.cwd());
        const dir = dirname(filePath);
        if (!existsSync(dir)) {
            mkdirSync(dir, { recursive: true });
        }
        ...
        for (const entry of branchEntries) {
            if (entry.type === "user") {
                exportData.messages.push({ role: "user", content: entry.content });
            } else if (entry.type === "agent") {
                exportData.messages.push({ role: "assistant", content: entry.content });
            }
        }
