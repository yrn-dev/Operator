...
function isOfflineModeEnabled() {
    const value = process.env.OPR_OFFLINE;
    if (!value)
        return false;
...
function collectSkillEntries(dir, mode, ignoreMatcher, rootDir) {
    ...
    try {
        ...
        for (const entry of dirEntries) {
            ...
            const relPath = toPosixPath(relative(root, fullPath));
            if (mode === "operator" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
                entries.push(fullPath);
                continue;
            }
            if (!isDir)
...
function readOperatorManifestFile(packageJsonPath) {
    try {
...
function resolveExtensionEntries(dir) {
    ...
    if (existsSync(packageJsonPath)) {
        const manifest = readOperatorManifestFile(packageJsonPath);
...
export class DefaultPackageManager {
    ...
    getNpmInstallArgs(specs, installRoot) {
        ...
        // Extension packages run inside operator and resolve operator APIs through loader aliases/virtual modules.
        // Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
        // equivalent bun/pnpm settings) so package managers do not install or solve host-provided
        // @yrn-dev/pzero-operator-* peers. Stale auto-installed operator peers can otherwise block updates.
    ...
    ensureNpmProject(installRoot) {
        ...
        if (!existsSync(packageJsonPath)) {
            const pkgJson = { name: "operator-extensions", private: true };
    ...
    collectPackageResources(packageRoot, accumulator, filter, metadata) {
        ...
        const manifest = this.readOperatorManifest(packageRoot);
        if (manifest) {
            for (const resourceType of RESOURCE_TYPES) {
    ...
    readOperatorManifest(packageRoot) {
    ...
    addAutoDiscoveredResources(accumulator, globalSettings, projectSettings, globalBaseDir, projectBaseDir) {
        ...
        if (projectTrusted) {
            ...
            // Project skills from .opr/
            addResources("skills", collectAutoSkillEntries(projectDirs.skills, "operator"), projectMetadata, projectOverrides.skills, projectBaseDir);
        }
        ...
        // User extensions from ~/.opr/agent/
        ...
        // User skills from ~/.opr/agent/
        addResources("skills", collectAutoSkillEntries(userDirs.skills, "operator"), userMetadata, userOverrides.skills, globalBaseDir);
