 ...
 */
import type { AgentMessage, AgentToolResult, AgentToolUpdateCallback, ThinkingLevel, ToolExecutionMode } from "@yernur/operator-agent-core";
import type { Api, AssistantMessageEvent, AssistantMessageEventStream, Context, ImageContent, Model, OAuthCredentials, OAuthLoginCallbacks, SimpleStreamOptions, TextContent, ToolResultMessage } from "@yernur/operator-ai";
import type { AutocompleteItem, AutocompleteProvider, Component, EditorComponent, EditorTheme, KeyId, OverlayHandle, OverlayOptions, TUI } from "@yernur/operator-tui";
...
export interface ExtensionUIContext {
     ...
     * extend `CustomEditor` from `@yernur/operator-agent` and call
     * `super.handleInput(data)` for keys you don't handle.
     *
     * @example
     * ```ts
     * import { CustomEditor } from "@yernur/operator-agent";
     *
...
export interface ExtensionContext {
    ...
    /** Gracefully shutdown operator and exit. Available in all contexts. */
    shutdown(): void;
...
export interface BeforeAgentStartEvent {
    ...
    systemPrompt: string;
    /** Structured options used to build the system prompt. Extensions can inspect this to understand what Operator loaded without re-discovering resources. */
...
export interface ExtensionAPI {
     ...
     * operator.registerProvider("my-proxy", {
     *   baseUrl: "https://proxy.example.com",
     *   apiKey: "$PROXY_AOPR_KEY",
     ...
     * operator.registerProvider("anthropic", {
     *   baseUrl: "https://proxy.example.com"
     * });
     *
     * @example
     * // Register provider with OAuth support
     * operator.registerProvider("corporate-ai", {
     ...
     *
     * @example
     * operator.unregisterProvider("my-proxy");
     */
    unregisterProvider(name: string): void;
    /** Shared event bus for extension communication. */
    events: EventBus;
}
/** Configuration for registering a provider via operator.registerProvider(). */
...
export type ExtensionFactory = (operator: ExtensionAPI) => void | Promise<void>;
...
/**
 * Action implementations for operator.* API methods.
