...
// Known context windows for providers that don't expose context length via their
// /models endpoint (e.g. Alem AI returns only id/object/created). Keyed by model id.
// Used when models.json doesn't specify contextWindow explicitly. Falls back to 128000.
...
function resolveContextWindow(modelId, provided) {
    if (provided !== undefined && provided > 0)
        return provided;
    if (modelId) {
        if (KNOWN_CONTEXT_WINDOWS[modelId] !== undefined)
            return KNOWN_CONTEXT_WINDOWS[modelId];
        const lower = modelId.toLowerCase();
        for (const key of Object.keys(KNOWN_CONTEXT_WINDOWS)) {
            if (key.toLowerCase() === lower)
                return KNOWN_CONTEXT_WINDOWS[key];
        }
        // Partial match for ollama model names (e.g. "kimi-k2.6:latest" → kimi)
        for (const key of Object.keys(KNOWN_CONTEXT_WINDOWS)) {
...
export class ModelRegistry {
    ...
    loadModels() {
        ...
        // Alem AI — 3 модели. qwen3-8 и gemma4 поддерживают image input; gpt-oss — text-only.
        const ALEM_AI_MODELS = [
            {
                id: "qwen3-8",
                name: "qwen3-8",
                provider: "alem-ai",
        ...
        for (const m of ALEM_AI_MODELS) {
            const exists = combined.some(x => x.provider === "alem-ai" && x.id === m.id);
            if (!exists) {
                combined.push(m);
            }
        }
        // Alem AI — ключ из auth.json или env
        this.storeProviderRequestConfig("alem-ai", {
            authHeader: true,
        });

        // Автообнаружение моделей Ollama (если сервер запущен)
        const OLLAMA_MULTIMODAL_HINTS = /\b(llava|bakllava|moondream|vision|vl|multimodal|cogito|minicpm|bunny|qwen.*vl|gemma.*it|x\/vision|granite.*vision)\b/i;
        try {
            const result = spawnSync("curl", ["-s", "http://localhost:11434/api/tags"], { encoding: "utf-8" });
                    ...
                    for (const m of data.models) {
                        ...
                        if (!exists) {
                            ...
                            combined.push({
                                ...
                                cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
                                contextWindow: resolveContextWindow(name, undefined),
                                maxTokens: 16384
                            });
                        }
                    }
            ...
            // Ollama не запущен — ничего не делаем
        }
        ...
        for (const [providerName, providerConfig] of Object.entries(config.providers)) {
            ...
            for (const modelDef of modelDefs) {
                ...
                models.push({
                    ...
                    contextWindow: resolveContextWindow(modelDef.id, modelDef.contextWindow),
    ...
    hasConfiguredAuth(model) {
        if (model.provider === "ollama" || model.provider === "lm-studio" || model.provider === "alem-ai") return true;
