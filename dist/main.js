...
const EXTENSION_LOAD_FAILURE_HINT = 'Hint: Start without extensions using "opr -ne".';
/**
...
export async function main(args, options) {
    resetTimings();
    const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.OPR_OFFLINE) || isTruthyEnvFlag(process.env.OPR_OFFLINE);
    if (offlineMode) {
        process.env.OPR_OFFLINE = "1";
        process.env.OPR_OFFLINE = "1";
        process.env.OPR_SKIP_VERSION_CHECK = "1";
    }
    ...
    if (await handlePackageCommand(args, { extensionFactories: options?.extensionFactories })) {
        ...
        if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
            ...
            // runs during teardown; let successful `operator update` drain naturally instead.
    ...
    const startupBenchmark = isTruthyEnvFlag(process.env.OPR_STARTUP_BENCHMARK);
    if (startupBenchmark && appMode !== "interactive") {
        console.error(chalk.red("Error: OPR_STARTUP_BENCHMARK only supports interactive mode"));
        process.exit(1);
    }
    if (appMode === "rpc") {
        printTimings();
